...
const Navbar = () => {
  ...
  const address = "0xab148ff7400"
  return (
    //ClassName to make menu respond on mobile devices 
    <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
      <div className="lg:flex-1 flex flex-row max-w-[458px] py-2 pl-4 pr-2 h-[52px] bg-[#1c1c24]
       rounded-[100px]">
        <input type="text" placeholder='Search here for compaigns...' 
        ...
        <div className="w-[72px] h-full rounded-[20px] bg-[#4acd8d] flex justify-center items-center
         cursor-pointer">
          ...
          className="w-[15px] h-[15px] object-contains"/>
        </div>
      </div>
      ...
      <div className="w-[40px] h-[40px] rounded-[10px] bg-[#2c2f32] flex justify-center items-center cursor-pointer">
          ...
          <img  src={menu}
            ...
            ${!toggleDrawer} ? '-translate-y-[100vh]' : 'translate-y-0]' transiction-all duration-700`}>
              <ul className='mb-4'>
                {navlinks.map((link) => (
                  <li
                  key={link.name}
                ...
                >
                  <img 
                    src={link.imgUrl}
                    alt={link.name}
                    className={`w-[24px] h-[24px] object-contain ${isActive === link.name ? 
