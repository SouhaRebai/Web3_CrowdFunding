...
const CampaignDetails = () => {
  //transfer state through routing w/ react DOM => useLocation
  const {state} = useLocation();
  const navigate = useNavigate();

  const {donateToACampaign, getDonations, contract, address} = useStateContext();
  ...
  const handleDonate = async () => {
    //async because we will be making a contract call
    setIsLoading(true);
    //console.log(state.pId)
    await donateToACampaign(state.pId,amount);
    navigate('/');
    setIsLoading(false);
  }
    ...
    <div className="mt-[60px] flex lg:flex-row flex-col gap-5">
          ...
          <div>
            ...
            <div className="mt-[20px] flex flex-row items-center flex-wrap gap-[14px]">
              ...
              <div>
                ...
                <p className="mt-[4px] font-epilogue font-normal text-[12px] text-[#808191]">A number of campaigns</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1">
          ...
          <div className="mt-[20px] flex flex-col p-4 bg-[#1c1c24] rounded-[10px]">
            <p className="font-epilogue fount-medium text-[20px] leading-[30px] 
